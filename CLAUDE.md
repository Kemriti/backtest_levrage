...
A single self-contained HTML file (`public/index.html`, ~6300 lines) implementing a browser-based backtest engine for a "Buy The Dip" leveraged-ETF trading strategy (UPRO, S&P 500 ×3), plus one small Cloudflare Worker (`src/worker.js`) that serves that static file and proxies Yahoo Finance for the auto-fetch feature. There is no build system, no package manager, and no test suite for the HTML — it's a static page with CDN-loaded libraries and an inline Python engine executed client-side via Pyodide. See `README.md` for the full strategy/feature description.

The project is deployed on **Cloudflare Workers (with static assets)**, connected to this GitHub repo: every `git push` to `main` runs `npx wrangler deploy` automatically, driven by `wrangler.jsonc` (`assets.directory: ./public`, `main: src/worker.js`). This is the modern Cloudflare deploy model — distinct from "classic Pages" (`_redirects`/`functions/` directory conventions), which this project does **not** use; don't reintroduce those files, they're silently ignored by `wrangler deploy`.

## Commands

There is no build, lint, or test tooling — there is nothing to compile or transpile.

- **Run the app locally**: open `public/index.html` directly in a browser. The Yahoo Finance auto-fetch will fall back to public CORS proxies (`corsproxy.io`, `allorigins`) since `/api/yahoo` (served by `src/worker.js`) only exists once deployed — or use manual CSV upload, or the built-in GBM-simulated dataset. `lancer_backtest.bat` (Windows) is a legacy fully-offline option: it spins up a tiny inline Node CORS proxy on `localhost:8080` before opening the page, but is no longer needed once the site is deployed.
...
- **Iterate on the Worker**: `npx wrangler dev` runs `src/worker.js` + the `public/` assets locally (needs Node + the `wrangler` package, fetched on demand via `npx`) if you need to validate `/api/yahoo` before pushing; otherwise just push and check the deployed URL.
- **Deploy manually**: `npx wrangler deploy` (same command Cloudflare's CI runs on push). Requires being logged in via `npx wrangler login`.
...
The repo has two parts: the monolith HTML app, and a one-file Worker (`src/worker.js`) that does two things based on request path — `/api/yahoo` forwards `?ticker=&period1=&period2=` to `query1.finance.yahoo.com/v8/finance/chart/...` server-side and re-emits the JSON with `Access-Control-Allow-Origin: *`; everything else falls through to `env.ASSETS.fetch(request)`, which serves `public/index.html` as the default document. If you change the query params `fetchYahooData()` sends in the HTML, update `handleYahoo()` in the Worker to match.

Everything else lives in one file (`public/index.html`), organized top-to-bottom as: `<style>` → HTML markup (upload/config UI + tab shells) → one big `<script>` with all JS, which dynamically generates and runs a Python program. Rough line map (will drift as the file is edited, but useful for orientation):
...
- `~1900` onward: JS. Key entry points:
  ...
  - `fetchAllYahoo()` (~6221) / `fetchYahooData()` (~6111) — auto-fetches UPRO/SPY/VIX from Yahoo's `v8/finance/chart` API. Tries proxies in order: own `/api/yahoo` (relative path → only resolves when served by the deployed Worker), then `corsproxy.io`, then `allorigins`. Each is a same-shape JSON passthrough of Yahoo's response, so parsing logic doesn't need to branch per-proxy.
